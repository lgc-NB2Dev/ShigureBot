...
async def get_img_msgs(info_photos, markdown=False):
    ...
    pics = []
    if info_photos:
        img_msg = await asyncio.gather(*[asyncio.create_task(get_img_msg(i)) for i in info_photos])
        if markdown:
            pics.append('- 证据图片：  ')
        for pic in img_msg:
...
async def send_info_msg(bot: Bot, ev: GroupMessageEvent | MessageEvent, **kwargs):
    ...
    async def parse_info(*args, **kws):
        return ((await get_msg(await parse_info_group_forward(*args, **kws))) if group_forward
