    ...
    if str(event.user_id) in bot.config.superusers and config.auto_allow_superusers:
        if not (e := await approve(event, bot)):
            ...
            await bot.send_private_msg(
                user_id=event.user_id, message=f'自动同意请求失败：{e.info["wording"]}'
            )
...
async def _(bot: Bot, event: GroupRequestEvent):
    ...
    if str(event.user_id) in bot.config.superusers and config.auto_allow_superusers:
        ...
        tmp[rdm := get_random_str()] = event
        await send_to_all_admins(bot, f"{sample}\n" f'{agree_sample.format(rdm, "")}')
...
async def _(bot: Bot, event: GroupIncreaseNoticeEvent):
    if event.is_tome() and config.send_addition_notice:
        await send_to_all_admins(
            bot, f"新添加群 {await get_group_name(bot, event.group_id)}({event.group_id})"
        )
...
async def _(bot: Bot, event: GroupDecreaseNoticeEvent):
    if event.is_tome() and config.send_leave_notice:
        ...
        await send_to_all_admins(
            bot,
            f"{action_name}群 {await get_group_name(bot, event.group_id)}({event.group_id})",
        )
...
async def _(bot: Bot, event: FriendAddNoticeEvent):
    if config.send_addition_notice:
        await send_to_all_admins(
            bot, f"新添加好友 {await get_stranger_name(bot, event.user_id)}({event.user_id})"
        )


@on_command(
    allow_cmd, aliases={refuse_cmd}, permission=SUPERUSER | PRIVATE_FRIEND
).handle()
async def _(
    bot: Bot,
    event: MessageEvent,
    ...
    if req := tmp.get(rdm):
        tip_sample = (
            f"管理员 {event.sender.nickname}({event.sender.user_id}) 已{approve_str}"
        )
        try:
            await process(req)
            req_name = "好友" if isinstance(req, FriendRequestEvent) else "邀请进群"
            await matcher.send(
                f"已{approve_str}该{req_name}请求"
        ...
        await matcher.send(
            tip_sample := f"已批量{approve_str} {len(reqs)} 个请求，成功 {success} 个，失败 {fail} 个"
        )
        await send_to_all_admins(
            bot,
