...
async def _(bot: Bot, matcher: Matcher, event: MessageEvent, arg: Message = CommandArg()):
    ...
    pic: list[MessageSegment] = event.reply.message["image"] if event.reply else event.message["image"]
    if not pic:
    ...
    if isinstance(msgs, list):
        if config.group_forward:
            ...
            msgs = [MessageSegment.node_custom(int(bot.self_id), self_nick, x) for x in msgs]

            if isinstance(event, GroupMessageEvent):
                return await bot.send_group_forward_msg(group_id=event.group_id, messages=msgs)
            elif isinstance(event, PrivateMessageEvent):
                return await bot.send_private_forward_msg(user_id=event.user_id, messages=msgs)
